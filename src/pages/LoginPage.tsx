...
export default function LoginPage() {
  ...
  const [customName, setCustomName] = useState('Zaydeli De La Rosa');
  ...
  const handleDirectOperatorLogin = (agentName: string, role: string = 'tlmk') => {
    const operator = {
      uid: `op_${Date.now()}`,
      name: agentName,
      ...
      <div className="mt-8 sm:mx-auto sm:w-full sm:max-w-md relative z-10">
        <div className="bg-[#0D121D]/90 backdrop-blur-xl py-8 px-6 shadow-2xl rounded-3xl sm:px-10 border border-slate-800 space-y-6">
          ...
          <div className="space-y-3">
            ...
            <button
              onClick={() => handleDirectOperatorLogin(customName || 'Zaydeli De La Rosa', 'tlmk')}
            ...
            >
              <PhoneCall className="w-4 h-4" /> Entrar como {customName || 'Agente TLMK'}
            </button>

            <div className="grid grid-cols-2 gap-2 pt-1">
              <button
                onClick={() => handleDirectOperatorLogin('Zaydeli De La Rosa', 'tlmk')}
              ...
              >
                <UserCheck className="w-3.5 h-3.5 text-[#00F0FF]" /> Zaydeli (TLMK)
              </button>
              <button
